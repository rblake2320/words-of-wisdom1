...
- **Manus platform coupling** — auth (Manus OAuth), owner notifications, and env injection all assume the Manus runtime. Standalone runs: copy `.env.example` to `.env`; login will not work outside Manus.
- **Notifications reach the owner only** — `admin.sendDailyNotification` sends an owner digest with the subscriber count. Real subscriber email delivery needs an email provider (Resend/SES); never put subscriber email addresses in notification content.
